...
def prevalidate_sql(sql: str, allowed_tables: List[str], db_type: str = "mysql") -> Dict[str, Any]:
    """
    SQL 预验证 - 在执行前进行全面检查
    
    整合语法检查、表名验证、反模式检测、资源限制检查。
    ...
    Returns:
        Dict: {
            "valid": bool,
            "can_execute": bool,  # 是否可以尝试执行
            "errors": List[str],  # 阻止执行的错误
            "warnings": List[str],  # 警告（不阻止执行）
            "suggestions": List[str],  # 修复建议
            "fixed_sql": str | None,  # 自动修复后的 SQL（如添加 LIMIT）
        }
    """
    result = {
        "valid": True,
    ...
    # 0. 使用新的 SQL 验证器进行综合验证
    try:
        from app.services.sql_validator import sql_validator
        
        # 构建简化的 schema_context 用于验证
        schema_context = {
            "tables": [{"table_name": t} for t in allowed_tables],
            "columns": []  # 列验证在后续步骤中进行
        }
        ...
        # 如果有自动修复的 SQL（如添加 LIMIT）
        if validation_result.fixed_sql:
            result["fixed_sql"] = validation_result.fixed_sql
            
    except ImportError:
        # sql_validator 模块不存在时，继续使用原有验证逻辑
        pass
    except Exception as e:
        # 验证器出错时记录警告，继续使用原有验证逻辑
        result["warnings"].append(f"SQL 验证器异常: {str(e)[:50]}")
    ...
    if not safety_result["valid"]:
        result["valid"] = False
        result["can_execute"] = False
        result["errors"].append(f"安全检查失败: {safety_result['error']}")
        return result
    
    # 3. 表名验证
    table_result = validate_sql_tables(sql, allowed_tables)
    if not table_result["valid"]:
        result["valid"] = False
        result["can_execute"] = False  # Phase 2: 表名验证失败时阻止执行
        result["errors"].append(table_result["error"])
        
        # 添加建议
        for invalid_table in table_result["invalid_tables"]:
            suggestion = suggest_similar_table(invalid_table, allowed_tables)
            if suggestion:
                result["suggestions"].append(f"'{invalid_table}' 可能应该是 '{suggestion}'")
    
    # 4. 反模式检测
    antipatterns = check_sql_antipatterns(sql, db_type)
    for issue in antipatterns:
        if issue["severity"] == "error":
            result["valid"] = False
            result["can_execute"] = False
            result["errors"].append(f"{issue['pattern']}: {issue['description']}")
            result["suggestions"].append(issue["suggestion"])
        else:
            result["warnings"].append(f"{issue['pattern']}: {issue['description']}")
            result["suggestions"].append(issue["suggestion"])
