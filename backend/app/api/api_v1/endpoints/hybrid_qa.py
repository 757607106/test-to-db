...
async def create_qa_pair_from_feedback(
    ...
    try:
        logger.info(f"[Feedback] 收到用户反馈 - 类型: {request.feedback_type}, "
                   f"连接ID: {request.connection_id}, "
                   f"会话ID: {request.thread_id or 'N/A'}, "
                   f"问题长度: {len(request.question)}, "
                   f"SQL长度: {len(request.sql)}")
        
        if request.feedback_type == "thumbs_up":
            # 点赞：创建问答对并存储
            logger.info(f"[Feedback-ThumbsUp] 开始创建问答对...")
            ...
            logger.debug(f"[Feedback-ThumbsUp] 提取表名: {used_tables}, 实体: {mentioned_entities}")
            
            # 分类查询类型
            query_type = classify_query_type_from_sql(request.sql)
            logger.debug(f"[Feedback-ThumbsUp] 查询类型: {query_type}")
            ...
            logger.info(f"[Feedback-ThumbsUp] 创建问答对对象 - ID: {qa_id}, "
                       f"表数量: {len(used_tables)}, 查询类型: {query_type}")
            ...
            logger.info(f"[Feedback-ThumbsUp] ✓ 问答对创建成功 - ID: {qa_pair.id}, "
                       f"问题: '{request.question[:50]}...', "
                       f"耗时: {elapsed:.2f}s")
            ...
            # 点踩：记录反馈用于后续改进分析
            elapsed = time.time() - start_time
            logger.warning(f"[Feedback-ThumbsDown] 收到负面反馈 - "
                         f"问题: '{request.question[:50]}...', "
                         f"SQL: '{request.sql[:50]}...', "
                         f"连接ID: {request.connection_id}, "
                         f"会话ID: {request.thread_id or 'N/A'}")
            
            # TODO: 可以将负面反馈存储到单独的表中，用于分析和改进
            # 目前仅记录日志
            logger.info(f"[Feedback-ThumbsDown] 反馈已记录，耗时: {elapsed:.2f}s")
        ...
        elapsed = time.time() - start_time
        logger.error(f"[Feedback-Error] 处理用户反馈失败 - "
                    f"类型: {request.feedback_type}, "
                    f"连接ID: {request.connection_id}, "
                    f"错误: {str(e)}, "
                    f"耗时: {elapsed:.2f}s", 
                    exc_info=True)
