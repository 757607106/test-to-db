...
def create_custom_analyst_agent(
    ...
    Returns:
        ChartGeneratorAgent实例
    """
    try:
        logger.info(f"Creating custom analyst agent from profile: {profile.name}")
        ...
        logger.info(
            f"Successfully created custom analyst agent: {profile.name}, "
        ...
        # 回退到默认智能体
        logger.warning("Falling back to default chart generator agent")
