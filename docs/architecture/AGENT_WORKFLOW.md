...
**重要说明**: 本文档基于实际代码实现编写 (2026-01-25更新)

## 一、实际的系统架构

### 1.1 真实的图结构

**关键发现**: 系统没有独立的控制流节点，所有路由逻辑都在 `supervisor_route()` 函数中实现。

```python
# 实际的图结构 (backend/app/agents/chat_graph.py)
...
用户输入查询
    ↓
🎯 [Entry Point: schema_agent 节点]
    ↓
📊 supervisor_route() 函数 (路由决策中心)
    │
    ├─ 检查意图 (闲聊关键词) → general_chat 节点 → END
    ├─ 检查缓存命中 → END (直接返回)
    ├─ 检查完成状态 → END
    └─ 基于 current_stage 路由到相应 Worker Agent
    ↓
Worker Agents 执行流程:
    ↓
1. schema_agent (分析Schema)
    ↓
2. clarification (可选，使用interrupt暂停)
    ↓
3. sql_generator (生成SQL，内置样本检索)
    ↓
4. sql_executor (执行SQL)
    ├─ 成功 → 5
    └─ 失败 → error_recovery → 重试或终止
    ↓
5. data_analyst (数据分析，总是执行)
    ↓
6. chart_generator (可选，快速模式跳过)
    ↓
7. recommendation (问题推荐)
    ↓
END (返回结果)
```

### 1.3 核心路由函数 (supervisor_route)

**这是系统的真正"大脑"**，所有路由决策都在这里完成：
...
def supervisor_route(state: SQLMessageState) -> str:
    ...
    承担职责:
    1. 意图检测 (闲聊 vs 数据查询)
    2. 缓存检查 (thread_history_hit, cache_hit)
    3. 阶段路由 (基于 current_stage)
    4. 错误处理 (重试决策)
    5. 完成检测
    """
    ...
    if current_stage == "init":
        # 检查闲聊关键词
        chat_keywords = ["你好", "谢谢", "帮助", "你是谁", "hello", "hi"]
    ...
    stage_routes = {
        ...
        "analysis_done": "chart_generator" or "recommendation",
...
**重要**: 没有独立的缓存检查节点，缓存逻辑内嵌在路由函数中。

**缓存标志位**：
- `thread_history_hit`: Thread 历史缓存命中
- `cache_hit`: 精确或语义缓存命中
- `cache_hit_type`: "exact" 或 "semantic"

**工作流程**：
1. 在进入主图前，外部逻辑设置缓存标志
2. `supervisor_route()` 检测到缓存命中，直接返回 "FINISH"
3. 不经过任何 Worker Agent，直接结束

---

## 二、Worker Agents 详解

### 2.1 实际存在的 Worker Agents

**文件位置**: `backend/app/agents/agents/`

| Agent | 文件 | 节点函数 | 职责 | Agent类型 |
|-------|------|---------|------|----------|
| **Schema Agent** | schema_agent.py | `schema_agent_node` | 分析查询意图，获取相关Schema | ReAct Agent |
| **Clarification** | clarification_agent.py | `clarification_node_wrapper` | 查询澄清 (使用interrupt) | 标准节点 |
| **SQL Generator** | sql_generator_agent.py | `sql_generator_node` | 生成SQL (内置样本检索) | ReAct Agent |
| **SQL Executor** | sql_executor_agent.py | `sql_executor_node` | 执行SQL，返回结果 | ToolNode |
| **Data Analyst** | data_analyst_agent.py | `data_analyst_node` | 数据分析和洞察生成 | ReAct Agent |
| **Chart Generator** | chart_generator_agent.py | `chart_generator_node` | 图表配置生成 | ReAct Agent |
| **Error Recovery** | error_recovery_agent.py | `error_recovery_node` | 错误分析和恢复 | 标准节点 |
| **Recommendation** | question_recommendation.py | `question_recommendation_node` | 推荐问题生成 | LLM调用 |
| **General Chat** | - | `general_chat_node` | 闲聊处理 | LLM调用 |

**已废弃的 Agents**:
- `sample_retrieval_agent` - 已集成到 `sql_generator_agent` 内部
- `sql_validator_agent` - 已移除

### 2.2 Worker Agents 执行流程

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                      supervisor_route() - 路由决策中心                       │
│  所有路由逻辑都在这个函数中实现，没有独立的控制流节点                          │
└─────────────────────────────────────────────────────────────────────────────┘
                                    │
                    ┌───────────────┴───────────────┐
                    │                               │
            数据查询 ▼                          闲聊 ▼
    ┌───────────────────────┐       ┌───────────────────────┐
    │  Worker Agents 流程    │       │  general_chat_node     │
    │        ↓              │       │  - 直接 LLM 回复       │
    └───────────────────────┘       │  - 返回 END            │
                │                   └───────────────────────┘
                ▼
...
│  - 实现：ReAct Agent + 异步并行获取                                         │
│  - 输出：schema_info, current_stage="schema_done"                          │
└─────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                      💬 clarification_node_wrapper (可选)                   │
│  - 职责：检测查询模糊性，使用 interrupt() 暂停等待用户回复                    │
│  - 实现：调用 clarification_node，支持 LangGraph interrupt                 │
│  - 输出：enriched_query, current_stage="clarification_done"                │
└─────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                      ⚙️ sql_generator_node                                  │
│  - 职责：生成SQL (内置样本检索功能)                                         │
│  - 实现：ReAct Agent + 结构化输出                                           │
│  - 特性：支持基于缓存模板生成、错误恢复上下文                                 │
│  - 输出：generated_sql, current_stage="sql_generated"                      │
└─────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                      🚀 sql_executor_node                                   │
│  - 职责：执行SQL查询                                                         │
│  - 实现：ToolNode包装（不使用ReAct）                                        │
│  - 优化：工具级缓存 + 并发锁                                                 │
│  - 输出：execution_result, current_stage="execution_done"                  │
└─────────────────────────────────────────────────────────────────────────────┘
                                    │
                    ┌───────────────┴───────────────┐
                    │                               │
               执行成功 ▼                      执行失败 ▼
    ┌───────────────────────────┐    ┌───────────────────────────┐
    │  进入数据分析              │    │  🔧 error_recovery_node    │
    │        ↓                  │    │  - 错误模式识别            │
    │  📊 data_analyst_node     │    │  - 恢复策略生成            │
    │  - 数据分析和洞察          │    │  - 设置重试或终止          │
    │  - 总是执行                │    └───────────────────────────┘
    └───────────────────────────┘                  │
                │                                  │
